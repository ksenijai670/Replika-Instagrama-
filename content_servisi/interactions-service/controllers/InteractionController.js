...
const RELATIONSHIP_SERVICE_URL =
...
async function getBlockStatus(userA, userB) {
  ...
  if (!response.ok) {
    throw new Error(
      "Greška pri komunikaciji sa relationship servisom (block-status)",
    );
  }
...
async function getRelationshipStatus(viewerId, ownerId) {
  ...
  if (!response.ok) {
    throw new Error(
      "Greška pri komunikaciji sa relationship servisom (relationship-status)",
    );
  }
...
const likePost = async (req, res) => {
  ...
  try {
    ...
    if (!allowed) {
      return res
        .status(403)
        .json({ error: "Nemate dozvolu da lajkujete ovu objavu" });
    }
...
const getLikesCount = async (req, res) => {
  ...
  try {
    ...
    if (!allowed) {
      return res
        .status(403)
        .json({ error: "Nemate dozvolu da vidite broj lajkova ove objave" });
    }
    ...
    for (const like of likes) {
      const blockedByOwner = await getBlockStatus(
        postMeta.userId,
        like.user_id,
      );
...
const addComment = async (req, res) => {
  ...
  try {
    ...
    if (!allowed) {
      return res
        .status(403)
        .json({ error: "Nemate dozvolu da komentarišete ovu objavu" });
    }

    const commentId = await InteractionModel.addComment(
      userId,
      postId,
      content,
    );
...
const updateComment = async (req, res) => {
  ...
  try {
    ...
    if (Number(comment.user_id) !== userId) {
      return res
        .status(403)
        .json({ error: "Možete menjati samo svoj komentar" });
    }
...
const deleteComment = async (req, res) => {
  ...
  try {
    ...
    if (!isCommentAuthor && !isPostOwner) {
      return res
        .status(403)
        .json({ error: "Nemate dozvolu da obrišete ovaj komentar" });
    }
...
const getCommentsByPost = async (req, res) => {
  ...
  try {
    ...
    if (!allowed) {
      return res
        .status(403)
        .json({ error: "Nemate dozvolu da vidite komentare ove objave" });
    }
...
const getCommentsCount = async (req, res) => {
  ...
  try {
    ...
    if (!allowed) {
      return res
        .status(403)
        .json({ error: "Nemate dozvolu da vidite broj komentara ove objave" });
    }
...
const deleteByPost = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Greška pri brisanju interakcija objave" });
  }
};
